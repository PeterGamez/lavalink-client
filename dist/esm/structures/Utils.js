...
export class ManagerUtils {
    ...
    validateQueryString(node, queryString, sourceString) {
        ...
        else if (SourceLinksRegexes.AllDeezerRegex.test(queryString) && !node.info?.sourceManagers?.includes("deezer")) {
            throw new Error("Lavalink Node has not 'deezer' enabled");
        }
        else if (SourceLinksRegexes.AllDeezerRegex.test(queryString) && node.info?.sourceManagers?.includes("deezer") && !node.info?.sourceManagers?.includes("http")) {
        ...
        else if (SourceLinksRegexes.pornhub.test(queryString) && !node.info?.sourceManagers?.includes("pornhub")) {
            throw new Error("Lavalink Node has not 'pornhub' enabled");
        }
        else if (SourceLinksRegexes.musicYandex.test(queryString) && !node.info?.sourceManagers?.includes("yandexmusic")) {
